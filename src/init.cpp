...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 53472, 53472));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, frequency, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all FREQUENCY specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizefrequencyamount=<n>", strprintf(_("Keep N FRQ anonymized (default: %u)"), 0));
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeFrequencyAmount = GetArg("-anonymizefrequencyamount", 0);
    if (nAnonymizeFrequencyAmount > 999999) nAnonymizeFrequencyAmount = 999999;
    if (nAnonymizeFrequencyAmount < 2) nAnonymizeFrequencyAmount = 2;
    ...
    LogPrintf("Anonymize FREQUENCY Amount %d\n", nAnonymizeFrequencyAmount);
